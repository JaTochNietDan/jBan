...
{
	$page = 'active';
	$result = mysql_query('SELECT *, TIMESTAMPDIFF(SECOND, NOW(), DATE_ADD(ban_timestamp, INTERVAL ban_time MINUTE)) AS ban_timeleft FROM `'.$MySQL_Table.'` WHERE NOW() <= DATE_ADD(ban_timestamp, INTERVAL ban_time MINUTE) OR ban_time = 0 ORDER BY '.$OrderReplace[$order_index].' '.$order_dir);
	echo 'Showing: Active | <a href="?show=expired">Expired</a> | <a href="?show=all">All</a>';
}
else if($_GET['show'] == "expired") 
{
	$page = 'expired';
	$result = mysql_query('SELECT *, TIMESTAMPDIFF(SECOND, NOW(), DATE_ADD(ban_timestamp, INTERVAL ban_time MINUTE)) AS ban_timeleft FROM `'.$MySQL_Table.'` WHERE NOW() >= DATE_ADD(ban_timestamp, INTERVAL ban_time MINUTE) AND ban_time != 0 ORDER BY '.$OrderReplace[$order_index].' '.$order_dir);
	echo 'Showing: <a href="/">Active</a> | Expired | <a href="?show=all">All</a>';
}
else if($_GET['show'] == "all") 
{
	$page = 'all';
	$result = mysql_query('SELECT *, TIMESTAMPDIFF(SECOND, NOW(), DATE_ADD(ban_timestamp, INTERVAL ban_time MINUTE)) AS ban_timeleft FROM `'.$MySQL_Table.'` ORDER BY '.$OrderReplace[$order_index].' '.$order_dir);
	echo 'Showing: <a href="/">Active</a> | <a href="?show=expired">Expired</a> | All';
}
else
{
	$page = 'active';
...
<table width="70%">
	...
	<?php
	if(mysql_num_rows($result) == 0) echo '<tr><td colspan="5">No bans yet!</td></tr>';
	else
	{
